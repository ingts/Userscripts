...
document.querySelectorAll('a[title="Report"]').forEach(rp => {
    ...
    document.querySelector(`#rp_${torrent_id}`).addEventListener('click', e => {
        ...
        e.currentTarget.closest("tr").insertAdjacentHTML('afterend', //language=html
            `
                <tr id="rp_helper">
                    <td>
                        ${comment_presets.length > 0 ? '<div style="margin: 0 auto 5px auto; width: 97%;display:flex;gap: 2px;" id="rp_helper_presets"></div>' : ''}
                        <form action="/reportsv2.php?action=takereport" enctype="multipart/form-data" method="post"
                              id="report_table">
                            <select style="margin: 0 auto 2px 12px;display:block;" name="type">
